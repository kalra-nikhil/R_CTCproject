...
modeldata_m01_prepost <- 
  rawdata |> 
  filter(!is.na(Cancer)) |> 
  select(Patient_ID,Cancer,Method01_pre,Method01_post) |> 
  pivot_longer(starts_with("Meth"),
               names_to="Time",
               values_to = "Count") |> 
  mutate(Time=str_remove(Time,"Method01_"))
ggplot(modeldata_m01_prepost,
       aes(Count, fill = Time))+
  geom_histogram(position="dodge")+
  facet_grid(rows = vars(Cancer))
model_out <- glmer(Count ~ Time * Cancer + (1|Patient_ID), 
                   family = poisson, 
...
model_summary <- summary(model_out)
model_summary
model_summary$coefficients

model_parameter <- exp(fixef(model_out))

model_confint <- exp(confint(model_out))

confint(model, method="profile", devtol=1e-4)
